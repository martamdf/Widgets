...
//
//func superCharge() {
//    if let heroes = getAllHeroes(groupName: appGroup, forKey: "heroes", castTo: [HeroDetail].self) {
//        for hero in heroes {
//            hero.healthLevel = 100
//            hero.isHealing = false
//            updateHero(groupName: appGroup, key: "heroes", hero: hero)
//        }
//    }
//}

func superCharge() {
    if var heroes = getAllHeroes(groupName: appGroup, forKey: "heroes", castTo: [HeroDetail].self) {
        // Actualizar todos los héroes en memoria
        heroes = heroes.map { hero in
            var modifiedHero = hero
            modifiedHero.healthLevel = 100
            modifiedHero.isHealing = false
            return modifiedHero
        }

        // Actualizar todos los héroes en el almacenamiento de datos una sola vez
...
func calculateHealingCompletionTime(currentHealth: Int) -> Date {
    let healingRatePerSecond = 1.0 / 2.0 // 1 punto de salud cada 2 segundos
    let healthToRecover = 100 - currentHealth
    let secondsToHeal = Double(healthToRecover) / healingRatePerSecond
    let completionTime = Date().addingTimeInterval(secondsToHeal)
