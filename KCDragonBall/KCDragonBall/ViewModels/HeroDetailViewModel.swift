...
final class HeroViewModel: ObservableObject {
    ...
    func startCounter() {
        do {
            startRecovery()
            activityIdentifier = try HeroActivityUseCase.startActivity(name: hero.name,
                                                      image: hero.photo,
                                                      hp: hero.healthLevel)
        } catch {
    ...
    func deleteActivity() {
        ...
        Task {
            await HeroActivityUseCase.endActivity(withActivityIdentifier: activityIdentifier)
        }
    }
